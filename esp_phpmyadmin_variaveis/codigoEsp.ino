...
void loop() {
  ...
  if(WiFi.status() == WL_CONNECTED) {
    ...
    http.begin("http://192.168.76.214/tcc/updateDHT11data.php");
    ...
    // Desligando o LED indicador de envio
    digitalWrite(ON_Board_LED, LOW);

    // Chamando as sub-rotinas para controle dos LEDs e obtenção de dados do sensor
    control_LEDs();
    get_DHT11_sensor_data();

    // Aguardando por 5 segundos
    delay(5000);
  }
}
